...
const MyReport = () => {
  ...
  const getReport = async (event) => {
    ...
    try {
      ...
      const { data } = await Axios.get('/report/adminReport', { params: query });
        ...
        <div className="card">
            ...
            <div className="row mt-4">
              <form onSubmit={getReport} >
                <div className="row">
                  ...
                  <div className="col">
                    <select className="form-select" onChange={(event) => setAgentId(event.target.value)}>
          ...
          <div className="card-body">
            ...
            <table className="table table-sm table-bordered table-responsive">
              <tbody>
                {
                  reports.map((agents, index) => (
                    <React.Fragment key={'agent_' + index}>
                      <tr>
                        <td scope='col' colSpan="20" className="text-center text-bg-secondary">{agents?.agent?.name}</td>
                      </tr>
                      {
                        agents?.clients.map((client, index) => (
